  ...
  <div class="member" v-loading="loading">
    ...
    <div class="contentoutline">
      <div class="contentleft">
        <div>
          <!-- <el-input placeholder="通过姓名或邮箱查找成员" v-model="filterText">
          </el-input> -->
          <div class="custom-tree-container">
            <div class="block">
              ...
              >
                <span
                  class="span-ellipsis custom-tree-node"
                  slot-scope="{ node, data }"
                >
                  <span class="treetext" :title="node.label">{{
                    node.label
                  }}</span>
                  <span class="treebtn">
                    <el-button
        ...
        <div>
          <div class="tablenew">
            ...
            >
              ...
              <el-table-column prop="creator" label="企业微信ID" width="100">
              </el-table-column>
    ...
    <confirmDialogalldelete
      :isShowDialogalldelete="isShowDialogdeletedep"
      :content="this.deletedepcontent"
      @on-result-changealldelete="changeIsShowDialogdeletedep"
      @child-operation="operationdeletedep"
    >
    </confirmDialogalldelete>
    <!--编辑部门-->
    <div class="mydialog mydialognew mydialogdeep">
      ...
      <el-dialog
        ...
        width="400px"
        :close-on-click-modal="false"
        :before-close="adddephandleClose"
      >
        <el-form
          ref="adddepdialogForm"
          :model="adddepdialogForm"
          label-width="90px"
          :rules="adddeprules"
        >
          <el-form-item label="部门名称" prop="name">
            <el-input
              v-model="adddepdialogForm.name"
              placeholder="请输入部门名称"
              :maxlength="20"
            ></el-input>
          </el-form-item>
        ...
        <div slot="footer" class="dialog-footer">
          <el-button @click="adddepcancel('adddepdialogForm')">取消</el-button>
          <el-button type="primary" @click="adddepsave('adddepdialogForm')"
            >确定</el-button
          >
        </div>
      </el-dialog>
    ...
    <div class="mydialog mydialognew mydialogdeep">
      ...
      <el-dialog
        ...
        width="400px"
        :close-on-click-modal="false"
        :before-close="editdephandleClose"
      >
        <el-form
          ref="editdepdialogForm"
          :model="editdepdialogForm"
          label-width="90px"
          :rules="editdeprules"
        >
          <el-form-item label="部门名称" prop="name">
            <el-input
              v-model="editdepdialogForm.name"
              placeholder="请输入部门名称"
            ></el-input>
          </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
          <el-button @click="editdepcancel('editdepdialogForm')"
            >取消</el-button
          >
          <el-button type="primary" @click="editdepsave('editdepdialogForm')"
            >确定</el-button
          >
        </div>
      </el-dialog>
...
export default {
  ...
  methods: {
    ...
    editrolesave() {
      console.log(this.editrolename);
      console.log(this.editrolesels);
    ...
    handleSelect(data, node) {
      ...
      if (data.isCompany != true) {
        temobj.entity.departmentId = departmentId;
      } else {
        temobj.entity.departmentId = "";
      }
      this.$http.team
        .getTeamList(temobj)
        .then(
    ...
    getCompanylist(params) {
      ...
      this.$http.team
        .getOrganization(temobj)
        .then(
          res => {
            if (res.data.ret == "0") {
              let temptreedata = res.data.content;
    ...
    addDepartment(node, data) {
      //console.log(data);
      //在顶层添加的时候，pId给0，不是公司id
      if(data.isCompany==true){
        this.adddepartment=0;
      }else{
        this.adddepartment =data.id;
      }
      this.adddepdialogVisible = true;
    },
    //编辑部门
    editDepartment(node, data) {
      this.editdepartment = node;
      this.editdepdialogForm.name = node.data.label;
      this.editdepdialogVisible = true;
    },
    //删除部门
    deleteDepartment(node, data) {
      this.deletedepartment = node.data.id;
      this.isShowDialogdeletedep = true;
      this.deletedepcontent = "确定要删除吗？";
    },
    ...
    changeIsShowDialogdeletedep(val) {
      this.isShowDialogdeletedep = val; //监听变化时触发的函数修改父组件的是否显示状态
    },
    ...
    operationdeletedep(type) {
      if (type == "confirm") {
        this.deletesaveDepartment();
        //点击确认要执行的代码
      } else if (type == "cancel") {
        this.deletedepartment = "";
        this.isShowDialogdeletedep = false;
        //点击取消要执行的代码
      }
    },
    //删除文件
    ...
    adddephandleClose() {
      this.$refs["adddepdialogForm"].resetFields();
      this.adddepartment = "";
      this.adddepdialogVisible = false;
    },
    editdephandleClose() {
      this.$refs["editdepdialogForm"].resetFields();
      this.editdepartment = {};
      this.editdepdialogVisible = false;
      this.editdepdialogForm = { name: "" };
    },
    // 删除按钮批量删除数据
    onDeldata() {
    ...
    adddepsave(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          let tempobjnew = this.adddepdialogForm;
          tempobjnew.pId = this.adddepartment;
          this.$http.team
            .addOrganization(tempobjnew)
            .then(
              res => {
                if (res.data.ret == "0") {
                  this.resetForm(formName);
                  this.adddepdialogForm = { name: "" };
                  this.adddepartment = "";
                  this.adddepdialogVisible = false;
                  this.$message({
                    message: "添加成功",
    ...
    editdepsave(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          let tempobjnew = this.editdepdialogForm;
          tempobjnew.id = this.editdepartment.data.id;
          tempobjnew.pId = this.editdepartment.parent.data.id;
          this.$http.team
            .editOrganization(tempobjnew)
            .then(
              res => {
                if (res.data.ret == "0") {
                  this.editdepdialogForm = { name: "" };
    ...
    //删除组织机构部门
    deletesaveDepartment() {
      let tempobjnew = { id: this.deletedepartment };
      this.$http.team
        .deleteOrganization(tempobjnew)
        .then(
          res => {
            if (res.data.ret == "0") {
              this.deletedepartment = "";
...
.contentoutline .contentright {
  float: left;
  /* width: 78%; */
  width: 80%;
}
.contentoutline .contentright .member-btns {
  background: #fff;
  padding-top: 13px;
  padding-left: 10px;
}
...
.custom-tree-container /deep/ .custom-tree-node .treebtn {
  float: right;
}
.custom-tree-container /deep/ .custom-tree-node .treetext {
  line-height: 28px;
  font-size: 13px;
  width: 80%;
  overflow: hidden;
...
.treebtn /deep/ .el-button:hover{
  border-color:none!important;
}
.treebtn .el-button:last-child{
  margin-right: 5px;
}
.treebtn .el-button.treeadd {
  background: url(../../assets/images/addicon.png) no-repeat;
  background-size: 100% 100%;
  margin-top: 5.5px;
}
.treebtn .el-button.treeedit {
  background: url(../../assets/images/file/editfiles.png) no-repeat;
  background-size: 100% 100%;
}
.treebtn .el-button.treedelete {
  background: url(../../assets/images/file/deletefiles.png) no-repeat;
  background-size: 100% 100%;
}
.contentleft /deep/ .el-tree-node__content:hover {
  background-color: #ecedfb !important;
}
.contentleft /deep/ .el-tree-node__content .treebtn {
  display: none;
}
.contentleft /deep/ .el-tree-node__content:hover .treebtn {
  display: inline-block;
}
.mydialog.mydialogdeep /deep/ .el-dialog .el-dialog__body {
  padding: 25px 20px 0px 20px;
}
.mydialog.mydialogdeep /deep/ .el-dialog .el-dialog__body .el-form-item {
  margin-bottom: 20px;
}
.el-table .cell span {
  display: initial !important;
}
.contentleft /deep/ .el-tree>.el-tree-node.is-expanded.is-focusable>.el-tree-node__content,.contentleft /deep/ .el-tree>.el-tree-node.is-focusable>.el-tree-node__content{
  background: #7b7de5;
  color: #ffffff;
}
.contentleft /deep/ .el-tree>.el-tree-node.is-expanded.is-focusable>.el-tree-node__content:hover,.contentleft /deep/ .el-tree>.el-tree-node.is-focusable>.el-tree-node__content:hover{
    background-color: #7b7de5!important;
    color: #ffffff;
}
.contentleft /deep/ .el-tree>.el-tree-node.is-expanded.is-focusable>.el-tree-node__content .el-icon-caret-right:before,.contentleft /deep/ .el-tree>.el-tree-node.is-focusable>.el-tree-node__content .el-icon-caret-right:before{
  color: #ffffff;
}
.contentleft /deep/ .el-tree>.el-tree-node.is-expanded.is-focusable>.el-tree-node__content .span-ellipsis.custom-tree-node .treetext,.contentleft /deep/ .el-tree>.el-tree-node.is-focusable>.el-tree-node__content .span-ellipsis.custom-tree-node .treetext{
  font-size:14px;
}
.contentleft /deep/ .el-tree>.el-tree-node.is-expanded.is-focusable>.el-tree-node__content .span-ellipsis.custom-tree-node .treebtn .el-button.treeadd,.contentleft /deep/ .el-tree>.el-tree-node.is-focusable>.el-tree-node__content .span-ellipsis.custom-tree-node .treebtn .el-button.treeadd{
  background: url(../../assets/images/addwhiteicon.png) no-repeat;
  background-size: 100% 100%;
  margin-top: 5.5px;
}
.contentleft /deep/ .el-tree>.el-tree-node.is-expanded.is-focusable>.el-tree-node__content .span-ellipsis.custom-tree-node .treebtn .el-button:hover,.contentleft /deep/ .el-tree>.el-tree-node.is-focusable>.el-tree-node__content .span-ellipsis.custom-tree-node .treebtn .el-button:hover{
  border-color:#7b7de5;
}
.inputbuttton {
  padding: 9px 10px;
